...
angular.module('vasaApp')
    .controller('PerencanaanPanduBaruCtrl', ['$scope', '$location', '$filter', '$timeout', '$window', '$interval', '$http', '$rootScope', 'API_PATH', 'KawasanPanduList', 'MonitoringAntrianPanduList', 'MonitoringPanduGenerateViewToTable','MonitoringAntrianPanduTestOpt', 'MonitoringPanduAdd', 'KawasanPanduLevelSatuList', 'SuratPerintahKerjaNumber', 'LoadingScreen', 'KapalTundaList', 'SuratPerintahKerjaPanduAdd', 'SuratPerintahKerjaPanduEdit', 'SuratPerintahKerjaPanduEditStatus', 'SuratPerintahKerjaPanduDetail', 'SuratPerintahKerjaPanduDelete', 'SuratPerintahKerjaPanduRpkro', 'Notification', 'PetugasPanduPerJadwalPandu', 'SpkTundaNumber', 'AlatApungList', 'SearchAlatApung', 'SpogNumber', 'PetugasPanduPerHariList', 'moment', 'SuratPerintahKerjaTambatAdd', 'AturanKapalTundaList', 'PenetapanPandu', 'PenetapanPanduEdit', 'MonitoringPetugasPandu', 'AppParam', 'KawasanPanduLevelDuaList', 'MdmDermagaSearchByKode', 'MdmDermagaPerJasa', 'AturanGerakPanduList', 'SuratPerintahKerjaPanduCancel', 'ParamsCabangList', '$PAGE_SIZE', 'PerencanaanPanduDelete', 'TipeEskalasiList', 'BindEskalasi', 'TipeEskalasi', 'SearchSDMKapal', 'RealisasiPanduDetailbyPpkJasa', 'SuratPerintahKerjaPanduResend', 'SpbInaportnet', 'SuratPerintahKerjaNumberPpkJasaTambat','PenetapanTambat', function ($scope, $location, $filter, $timeout, $window, $interval, $http, $rootScope, API_PATH, KawasanPanduList, MonitoringAntrianPanduList, MonitoringPanduGenerateViewToTable,MonitoringAntrianPanduTestOpt, MonitoringPanduAdd, KawasanPanduLevelSatuList, SuratPerintahKerjaNumber, LoadingScreen, KapalTundaList, SuratPerintahKerjaPanduAdd, SuratPerintahKerjaPanduEdit, SuratPerintahKerjaPanduEditStatus, SuratPerintahKerjaPanduDetail, SuratPerintahKerjaPanduDelete, SuratPerintahKerjaPanduRpkro, Notification, PetugasPanduPerJadwalPandu, SpkTundaNumber, AlatApungList, SearchAlatApung, SpogNumber, PetugasPanduPerHariList, moment, SuratPerintahKerjaTambatAdd, AturanKapalTundaList, PenetapanPandu, PenetapanPanduEdit, MonitoringPetugasPandu, AppParam, KawasanPanduLevelDuaList, MdmDermagaSearchByKode, MdmDermagaPerJasa, AturanGerakPanduList, SuratPerintahKerjaPanduCancel, ParamsCabangList, $PAGE_SIZE, PerencanaanPanduDelete, TipeEskalasiList, BindEskalasi, TipeEskalasi, SearchSDMKapal, RealisasiPanduDetailbyPpkJasa, SuratPerintahKerjaPanduResend, SpbInaportnet, SuratPerintahKerjaNumberPpkJasaTambat,PenetapanTambat) {
        $scope.items = [];
        ...
        $scope.jasaTambatAsalOption = {};
        $scope.jasaTambatTujuanOption = {};
       ...
       /* $scope.$watch('searchJenisGerakan', function(newValue) {
        ...
        $scope.setStatus = function(data){
            if(data == null){
                $scope.search.statusSpk = 'Available';
            } else {
                $scope.search.statusSpk = '';
            }
        ...
        $scope.loadAntrian = function(tanggalPanduAwal, tanggalPanduAkhir,newPage) {
            ...
            if(newPage>0){
                MonitoringAntrianPanduTestOpt.get({
                size: $scope.pageSize,
                page: newPage - 1,
                ...
                sort: $scope.sortBy == '' ? 'tglPenetapan,desc' : ($scope.sortBy + ',' + ($scope.sortDesc ? 'asc' : 'desc'))
            }, function(response) {
                ...
                $timeout(function() {
                    ...
                    $scope.monitoringData.forEach(function(data) {
                        ...
                        if (data.statusSpk === null) {
                            data.statusSpk = 'Available';
                        } else if (data.statusSpk === 'OHN') {
                            data.statusSpk = 'Konfirmasi';
                        }
                ...
                MonitoringPanduGenerateViewToTable.get({
                size: $scope.pageSize,
                page: newPage - 1,
                tglAwal: tanggalPanduAwal,
                tglAkhir: tanggalPanduAkhir,
        ...
        $scope.loadPerencanaan = function(tanggalPanduAwal, tanggalPanduAkhir,newPage) {
            var tanggalPanduAwal = moment($scope.tanggalPanduAwal, 'DD-MM-YYYY').format('YYYYMMDD');
            var tanggalPanduAkhir = moment($scope.tanggalPanduAkhir, 'DD-MM-YYYY').add(1, 'days').format('YYYYMMDD');
        ...
        $scope.showModalSpk = function(item) {
            ...
            $scope.spk.noPpkJasaPandu = item.noPpkJasaPandu;
        ...
        $scope.checkSpog = function (item) {
            if ($scope.entryInaportnet == '1') {
                SpogNumber.get({ noSpk: item.nomorSpk }, function (response) {
        ...
        $scope.showModalPenetapanPandu = function(item) {
            /*untuk mendapatkan penetapan tambat dari 
             *ppk1 yang dimasukan untuk jadi pilihan dropdown tambat asal dan tujuan
             */
            PenetapanTambat.get({
                noPpk1 : item.noPpk1
            },
            function (response)
            {
                $scope.jasaTambatAsalOption = response.content;
                $scope.jasaTambatAsalOption.push("");
                $scope.jasaTambatTujuanOption = response.content;
            });

            //memasukan item yang dikirim dari list ke penampung yang bernama $scope.dataPermohonan
            ...
            //isi $scope.dataPermohonan.jasaTambatAsal dan $scope.dataPermohonan.jasaTambatTujuan
            //dengan api penetapan pandu get dengan parameter noppkjasa
            PenetapanPandu.get({
                noPpkJasa : item.noPpkJasaPandu
            }, function(response){
                $scope.dataPermohonan.jasaTambatAsal = response.content[0].jasaTambatAsal;
                $scope.dataPermohonan.jasaTambatTujuan = response.content[0].jasaTambatTujuan;
            });
        };
        ...
        $scope.submitPenetapanPandu = function(item) {
            if(item.noPpkJasaRevisi == null){
                $scope.penetapanPandu.noPpkJasa = item.noPpkJasaPandu;
            } else {
            ...
            if (typeof item.jasaTambatAsal == 'object') {
                $scope.penetapanPandu.jasaTambatAsal = null;
            } else{
                $scope.penetapanPandu.jasaTambatAsal = item.jasaTambatAsal;
            }

            if (typeof item.jasaTambatTujuan == 'object') {
                $scope.penetapanPandu.jasaTambatTujuan = null;
            } else{
                $scope.penetapanPandu.jasaTambatTujuan = item.jasaTambatTujuan;
            }
            ...
            $scope.isBusy();
            $location.path("/manajemenpandu/perencanaanbaru");
        };
        ...
        $scope.swapUp = function(item, index) {
            ...
            if (index >= 1) {
                while (otherItemPos < index) {
                    otherItemPos++;
                    if (item[index - otherItemPos].statusSpk === 'Available') {
                        break;
                    }
                }
